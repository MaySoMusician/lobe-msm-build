...
test('opens and dismisses the working panel from mobile chat chrome', async ({
  ...
  if (!toggle) {
    throw new Error('Could not find mobile working-panel toggle');
  }
  await toggle.click();
  await expect(toggle).toBeHidden();
  const closePanel = await visibleButtonByIcon(body, ['lucide-panel-right-close']);
  if (!closePanel) {
    throw new Error('Could not find mobile working-panel close action');
  }
  ...
  await expect
    .poll(async () => Boolean(await visibleButtonByIcon(body, ['lucide-panel-right-open'])))
    .toBe(true);
});
